...
| ```reget```     | ```downloadF```     | In an FTP console, REGET (or restart with a marker) is a client command that resumes an interrupted file download from where it left off, checking for a partial local file and continuing transfer.     |
